...
function TeamProjectsDetails() {
  ...
  function handleToggle(taskId) {
    ...
    setIsDone(newIsDone)
    axios.put(`http://localhost:3000/tasks/updateStatus/${taskId}`, {isDone: newIsDone})
      .then((response) => {
        if (response.ok) {
        ...
        <ul>
          {tasks.map((task) => (
            <li key={task._id} className={`mb-4 p-4 rounded-lg shadow-md ${task.priority === 'low' ? 'bg-green-300' : task.priority === 'medium' ? 'bg-blue-300' : 'bg-red-300'}`}>
              ...
              <label className="relative inline-flex items-center cursor-pointer ml-96" onClick={() => handleToggle(task._id)}>
                <input
                  type="checkbox"
                  value=""
                ...
                <span className="ml-3 text-sm font-medium text-gray-700">Done!</span>
              </label>
